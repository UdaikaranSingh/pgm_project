...
if not os.path.exists('./results/experiment_3/'):
    os.mkdir('./results/experiment_3/')
...
def get_synthetic_summary_holdout(synthetic_data_func, n=1000, sd = 1, valid_size=0.2, k=1):
    ...
    """
    if LOAD_DATA:
        with open(f'results/experiment_3/preds_dict_train_n_{n}_sd_{sd}.pkl', 'rb') as handle:
            preds_dict_train = pickle.load(handle)
        with open(f'results/experiment_3/preds_dict_valid_n_{n}_sd_{sd}.pkl', 'rb') as handle:
            preds_dict_valid = pickle.load(handle)
        with open(f'results/experiment_3/summaries_train_n_{n}_sd_{sd}.pkl', 'rb') as handle:
            summaries_train = pickle.load(handle)
        with open(f'results/experiment_3/summaries_validation_n_{n}_sd_{sd}.pkl', 'rb') as handle:
        ...
        for i in range(k):
            preds_dict_train, preds_dict_valid = get_synthetic_preds_holdout(
                synthetic_data_func, n=n, sd = sd, valid_size=valid_size
            )
        ...
        with open(f'results/experiment_3/preds_dict_train_sd_n_{n}_sd_{sd}.pkl', 'wb') as handle:
            pickle.dump(preds_dict_train, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(f'results/experiment_3/preds_dict_valid_n_{n}_sd_{sd}.pkl', 'wb') as handle:
            pickle.dump(preds_dict_valid, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(f'results/experiment_3/summaries_train_n_{n}_sd_{sd}.pkl', 'wb') as handle:
            pickle.dump(summaries_train, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(f'results/experiment_3/summaries_validation_n_{n}_sd_{sd}.pkl', 'wb') as handle:
    ...
    plot_dist(preds_dict_train, S_learner_names, f'results/experiment_3/train_dist_n_{n}_p_{p}.png')
    plot_dist(preds_dict_valid, S_learner_names, f'results/experiment_3/valid_dist_n_{n}_p_{p}.png')
    distr_plot_single_sim(preds_dict_train, f'results/experiment_3/single_sim_dist_train_n_{n}_p_{p}.png', kind='kde', linewidth=2, bw_method=0.5,
                      drop_learners=[KEY_ACTUAL] + list(preds_dict_train.keys()))
    distr_plot_single_sim(preds_dict_valid, f'results/experiment_3/single_sim_dist_valid_n_{n}_p_{p}.png', kind='kde', linewidth=2, bw_method=0.5,
                      drop_learners=[KEY_ACTUAL] + list(preds_dict_train.keys()))
...
def get_synthetic_preds_holdout(
    synthetic_data_func, n=1000, sd = 1, valid_size=0.2, estimators={}
):
    ...
    """
    y, X, w, tau, b, e = synthetic_data_func(n=n, sigma = sd)
...
def experiment(n,sd):
    print(f"current experiment -  n: {n} sd: {sd}")
    ...
    train_summary.to_csv(f'results/experiment_3/train_summary_sd_{sd}_n_{n}.csv')
    validation_summary.to_csv(f'results/experiment_3/validation_summary_sd_{sd}_n_{n}.csv')
    print(f"Finished experiment -  n: {n} sd: {sd}")
